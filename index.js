  ...
  init() {
    ...
    if (this.callback && this.serviceCommands) {
      this.subscriber = new Subscriber(
        ...
        this.debug,
        (message, messageData) => {
          this.callback(message, messageData);
        }
      );
    } else {
      this.log(
        "Cannot create subscriber. The callback function is not defined."
      );
    }
  }
        ...
        this.serviceCommands.forEach((command) => {
          this.serviceResponses.forEach((response, index, object) => {
            if (response.channel === command.channel) {
              log(
                "Potential pubsub loop detected! Removing response: " +
                  response.friendlyName +
                  " with duplicate channel: " +
                  command.channel
              );
